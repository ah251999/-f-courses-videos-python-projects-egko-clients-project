...
def clients_list():
    ...
    add_new_product_but.config(text='تسجيل بيع بند',state='normal')
    ...
    remove_record_but.config(text='تخصيم', fg='black')
...
def myt_message(condi):
    ...
    if condi == 0:
        ...
        tk.messagebox.showwarning(title='!انتبه', message='برجاء ملء البيانات المطلوبة')
...
add_new_product_but = tk.Button(but_frame, text='تسجيل بيع بند',font=('bold',15))
add_new_product_but.grid(row=1,column=0,pady=6)

#anc_ri_but is shortname for add new client and remove item button
add_new_client_but = tk.Button(but_frame, text='إضافة عميل جديد', font=('bold',15))
add_new_client_but.grid(row=2,column=0,pady=6)

remove_record_but = tk.Button(but_frame, text='تخصيم', font=('bold',15))
